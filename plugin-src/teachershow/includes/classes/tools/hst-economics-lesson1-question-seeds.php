...
/**
 * Reviewed question bank for Grade 10 Humanities Economics, lesson one.
 *
 ...
 * textbook. It is installed once per active term on one canonical Economics
 * lesson; exam injection can transparently clone a selected question into a
 * compatible destination class when necessary.
 */
final class HST_Economics_Lesson1_Question_Seeds
{
    private const SUBJECT = 'g10-economics';
    private const CHAPTER = 'c1';
    private const TOPIC = 'c1-t1';
    private const BANK_ID = 'economics-lesson1-1404';
    private const DATA_REVISION = '1404-03';
    ...
    public static function seed_scope(int $term_id, string $grade, string $major): array
    {
        ...
        if ($term_id < 1 || $grade !== 'tenth' || $major !== 'humanities') {
            return $result;
        }
        ...
        foreach ((array) $rows as $row) {
            ...
            if (!self::is_economics((string) $row->lesson_name)) {
                continue;
            }
    ...
    public static function questions(): array
    {
        ...
        $records = require HST_PATH . 'includes/data/hst-economics-lesson1-question-bank.php';
    ...
    private static function is_economics(string $name): bool
    {
        ...
        return (bool) preg_match('/^اقتصاد(?:\s*[1۱])?$/u', $name);
    }
}
